...
public class LuceneSearchPlugin implements PlugableSearchIndex<LuceneInMemoryIndex, LuceneIndexConfig, Query>, Serializable, PortableObject {
	...
	public boolean evaluate(Query query, Object document) {
		AnalyzedDocument doc = (AnalyzedDocument) document;
		MemoryIndex memIndex = new MemoryIndex();
		for(Map.Entry<String, IndexableField> entry: doc.getFields().entrySet()) {
			String field = entry.getKey();
			float boost = entry.getValue().boost();
			TokenStream ts;
			try {
				ts = entry.getValue().tokenStream(null);
				memIndex.addField(field, ts, boost);
			} catch (IOException e) {
